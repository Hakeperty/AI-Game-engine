...
const host = await ProjectHost.create(dir, {
  name: 'coin-platformer',
  description: 'Floating-island coin platformer built by an AI through AIGE tools',
});
...
const island = await call('model_create', {
  name: 'sky-island',
  source: readFileSync(join(out, 'sky-island.model.ts'), 'utf8'),
});
...
for (const t of ['coin', 'flag', 'spike', 'mushroom', 'bush', 'tree-organic', 'flower', 'rock']) {
  ...
  console.log(
    t,
    r.size.map((n: number) => +n.toFixed(2)),
    r.triangles,
  );
}
await host.close();
