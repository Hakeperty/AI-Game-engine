...
const host = await ProjectHost.create(mkdtempSync(join(tmpdir(), 'aige-docs-')), { name: 'docs' }, { render: null });
const index: string[] = ['# AIGE documentation for AI agents', '', 'The same text is served by the `api_docs` tool and the `aige://docs/*` MCP resources.', ''];
