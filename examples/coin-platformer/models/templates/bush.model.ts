import { defineModel, icosphere, mixColor, model, Noise, p, PolyMesh, sdf, type V3 } from 'aige/model';
...
export default defineModel({
  name: 'bush',
  description: 'Leafy shrub made of soft, lumpy foliage clumps with optional berries or flowers. Sits on y = 0.',
  params: {
  ...
  build({ height, width, leaves, leaves2, clumps, extras, extraColor }, { rng, seed }) {
    ...
    const foliage = sdf
      .smoothUnionAll(balls.map((b) => sdf.sphere(b.radius, b.center)), 0.12)
    ...
    if (extras !== 'none') {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const c: V3 = [0, 0.45, 0];
        let t = 0.2;
        for (let k = 0; k < 40 && probe.distance([c[0] + dir[0] * t, c[1] + dir[1] * t, c[2] + dir[2] * t]) < 0; k++) t += 0.03;
        const pos: V3 = [c[0] + dir[0] * (t - 0.025), c[1] + dir[1] * (t - 0.025) - 0.02, c[2] + dir[2] * (t - 0.025)];
        if (pos[1] < 0.12) continue;
        if (extras === 'berries') {
          dots.push(icosphere({ radius: rng.range(0.025, 0.035), detail: 2 }).translate(pos).color(extraColor));
        } else {
    ...
    return model(parts).setCollider({ shape: 'sphere', radius: 0.55 * Math.max(sx, sy), offset: [0, 0.45 * sy, 0] });
  },
});
