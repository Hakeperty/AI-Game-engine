 ...
 */
export async function launchChromium(probe?: (page: Page) => Promise<void>): Promise<{ browser: Browser; page: Page; label: string }> {
  ...
  for (const a of attempts) {
    ...
    try {
      browser = await chromium.launch({ headless: true, args: [...a.args, '--allow-file-access-from-files'], ...(a.channel ? { channel: a.channel } : {}) });
