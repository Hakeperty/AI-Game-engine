...
- \`character_create\` {name, preset: 'young_man'|'boy'|'woman'|'none', params} writes models/<name>.model.ts (templates human, young_man, boy, woman). In a recipe: humanoid({ preset, height, age: 'adult'|'teen'|'child', sex, build, muscle, headShape, skin, eyes, hair: 'messy'|'short'|'buzz'|'shoulder'|'bun'|'none', hairColor, top: 'hoodie'|'tshirt'|'sweater', bottom: 'jeans'|'shorts'|'trousers', shoes: 'boots'|'sneakers'|'shoes'|'barefoot', *Color, wear, stubble, clips, detail }).
