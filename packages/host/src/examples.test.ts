...
describe.skipIf(examples.length === 0)('example replays', () => {
  it.each(examples)('%s replays to identical scenes', async (name) => {
    ...
    const log = readFileSync(join(dir, 'commands.jsonl'), 'utf8')
      ...
      .filter(Boolean)
      .map((l) => JSON.parse(l));
    const host = await ProjectHost.create(mkdtempSync(join(tmpdir(), 'aige-replay-')), { name: project.name }, { render: null });
    try {
