...
describe('result conversion', () => {
  it('moves images into image content and strips base64 from the text', () => {
    const r = toCallToolResult({
      ...
      result: {
        size: [0.1 + 0.2, 1, 2],
        images: [{ mimeType: 'image/png', data: 'AAAA', label: 'preview', path: '.aige/x.png' }],
      },
    });
    expect(r.content).toHaveLength(2);
    expect(r.content[0]).toEqual({
      type: 'text',
      text: '{"images":[{"label":"preview","mimeType":"image/png","path":".aige/x.png"}],"size":[0.3,1,2]}',
    });
  ...
  it('reports errors with isError and the hint', () => {
    const r = toCallToolResult({
      ok: false,
      error: { code: 'NOT_FOUND', message: 'nope', hint: "Did you mean 'Player'?" },
    });
...
describe('stdio server', () => {
  it('serves tools to an MCP client', async () => {
    ...
    expect(tools.map((t) => t.name)).toEqual(
      expect.arrayContaining(['project_create', 'entity_create', 'model_create', 'render_screenshot']),
    );
    const created = (await client.callTool({ name: 'project_create', arguments: { name: 'demo' } })) as {
      isError?: boolean;
    };
    expect(created.isError).toBeFalsy();
    const tree = (await client.callTool({ name: 'scene_tree', arguments: {} })) as {
      content: { text: string }[];
    };
