...
## 0. Latest update (2026-09-24, morning): read this first

**Play it:** double-click **`Play The Sidle of Milch.bat`** at the repo root. It runs `games/sidle-of-milch/dist/SidleOfMilch.exe` if you have exported it, otherwise the project in Godot. Rebuild the exe with `godot --headless --path games/sidle-of-milch --export-release "Windows Desktop" dist/SidleOfMilch.exe`; the export templates are installed in `%APPDATA%/Godot/export_templates/4.7.2.stable.mono`.

**New since the first hand-off:**
- **Chapter 1 plays end to end in Godot:**
  - C# game logic in `scripts/Chapter1.cs` (the opening, objectives with waypoints, stairs unlocking after the ground floor is explored, faint → morning, leaving the cabin).
  - All 8 cutscenes, generated by `build/cutscenes.ts`: wake, photo, hall, kitchen with thunder, cabinet, empty room with the faint, morning, leave.
  - Milch is the real skinned character with animations.
  - Verified by a play-test: the CS1 events, voice lines, flags and objectives fire in order with no errors.
- **Resident Evil-style look:** a darker, desaturated green-teal grade, heavy grain, chromatic aberration (new Environment `chromaticAberration`, applied in the Hud shader), and a tighter over-the-shoulder camera.
- **Ireland and meaning:** the Irish home details and story props from STORY.md's "Lore" section are placed:
  - milk saucer, horseshoe, Brigid's cross, holy water font, Sacred Heart with its red lamp;
  - the calendar stopped at the 14th, hawthorn in a jar, and Murphy's drawing with the scribbled-out face;
...
- **Bug fixes:**
  - Inactive entities stay silent (the birds no longer play at night).
  - `godot_screenshot` passes `--aige-no-cutscenes`.
  - The Unity folder is excluded from the Godot build.
  - Game exports go to `dist/`, and `build/` (the builder scripts) is now tracked in git.
- **In progress when the usage limit hit**; resume both, they have clear briefs:
  - **Unity 6 export** (`packages/unity`, `games/sidle-of-milch/unity/`): the Unity project and packages are resolved; the TypeScript exporter and C# runtime are only partly written. The plan is in the agent brief: URP, glTFast, an editor importer that builds `.unity` scenes, and a C# runtime mirroring the Godot one.
  - **Character quality pass** (`packages/modeling/src/character/`): realistic proportions, clothing folds, a hood, hair, fabric materials (`materials/cloth_*` fetched), and animation weight and foot-sliding fixes. Verify it in Godot as well as in previews.
